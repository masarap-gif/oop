...
import static DSA.LinkedlistBook.quickSortByGenre;
...
public class BORROWINGUI extends JFrame {
   ...
   public static arryList transacs;
    ...
    public BORROWINGUI(LinkedlistBook list) {
        try {
    ...
    // Sort the books by genre before populating the table
    NodeBook[] booksArray = book.toArray();   // Convert the LinkedList to an array
    quickSortByGenre(booksArray, 0, booksArray.length - 1);  // Sort by genre
    
    // Now populate the table with the sorted data
    populateTable(selectedGenre, booksArray);  // Assuming populateTable can accept the sorted array
});
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (selectedRow != -1) {
            ...
            int isbn = Integer.parseInt(isbnString);  // Convert to int if needed
...
public void actionPerformed(ActionEvent e) {
    try {
       ...
       System.out.println(dueDateStr);
        ...
        // Add transaction
        arryList.addTransaction(userid, isbn, transacs, dueDate , borrowDate);
        
        tableModel.addRow(new Object[]{userid, isbn, transacs, borrowDate, dueDate});
...
table.getColumnModel().getColumn(4).setCellRenderer(new DefaultTableCellRenderer() {
    ...
    protected void setValue(Object value) {
        ...
        LinkedlistBook.updateAvailability(isbn, false);
        updateTable();
        ...
        JOptionPane.showMessageDialog(null, "An error occured pls try again");
    }
    }
    ...
    public void actionPerformed(ActionEvent e) {
     ...
     public void populateTable(String selectedGenre, NodeBook[] booksArray) {
    // Filter books by the selected genre and populate the table
    List<NodeBook> filteredBooks = new ArrayList<>();
    for (NodeBook book : booksArray) {
        if (book.getGenre().equals(selectedGenre)) {
            filteredBooks.add(book);
        }
    }
    // Convert filteredBooks to Object[][] for the table and display
    Object[][] data = new Object[filteredBooks.size()][5];
    for (int i = 0; i < filteredBooks.size(); i++) {
        NodeBook book = filteredBooks.get(i);
        data[i][0] = book.getTitle();
        data[i][1] = book.getAuthor();
        data[i][2] = book.getISBN();
        data[i][3] = book.getGenre();
        data[i][4] = book.getIsAvailable();
    }
    // Assume tableModel is your JTable's model
